...
void pushSendDataToQueu(void* i_SqsockInfo, int iWriteSize, unsigned char *uchWriteBuff ){
	int iPushCnt;
	QData* qData;
	QSOCKINFO* SqsockInfo = (QSOCKINFO*)i_SqsockInfo;
	status_t status=0;
...
void* commandGroup(void* i_SqsockInfo)
{
	...
	while(1){
		...
		if(uiCount > 0){
			...
			puchReadData = uchReadBuff;
			puchWriteData = uchWriteBuff;
			iWriteSize = ICDHandler(puchWriteData, puchReadData, iWriteSize);
			iWriteSize = iWriteSize;
			if(iWriteSize >= 0){
				pushSendDataToQueu(SqsockInfo, iWriteSize, puchWriteData);
			}
		}else{
			usleep(1000);
		}
...
void* recvCommand(void* i_SqsockInfo)
{
	...
	while(1){
		...
		if(iRecvSize <= 0){
			...
			printLog( "by pcw : recv from Client msg : %s\n",uchReadBuff);
